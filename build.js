...
// Read vendored Meteocons fill weather icons (MIT, Bas Milius).
// Served by the Worker at /icons/weather/<name>.svg so the single-Worker
// app never hotlinks a third-party icon CDN at runtime.
const weatherIconsDir = path.join(__dirname, 'public', 'icons', 'weather');
let weatherIconFiles = {};
if (fs.existsSync(weatherIconsDir)) {
  for (const file of fs.readdirSync(weatherIconsDir).sort()) {
    if (file.endsWith('.svg')) {
      weatherIconFiles[file] = fs.readFileSync(path.join(weatherIconsDir, file), 'utf8');
    }
  }
}
...
let cardIconFiles = {};
if (fs.existsSync(cardIconsDir)) {
  for (const file of fs.readdirSync(cardIconsDir).sort()) {
    if (file.endsWith('.svg')) {
      cardIconFiles[file] = fs.readFileSync(path.join(cardIconsDir, file), 'utf8');
    }
  }
}

// Read vendored Meteocons fill alarm icons (MIT, Bas Milius).
// Animated fill set, same family as the forecast icons above, used only
// for NWS alert headers (see public/icons/alerts/). Served by the Worker
// at /icons/alerts/<name>.svg so the single-Worker app never hotlinks a
// third-party icon CDN at runtime.
const alertIconsDir = path.join(__dirname, 'public', 'icons', 'alerts');
let alertIconFiles = {};
if (fs.existsSync(alertIconsDir)) {
  for (const file of fs.readdirSync(alertIconsDir).sort()) {
    if (file.endsWith('.svg')) {
      alertIconFiles[file] = fs.readFileSync(path.join(alertIconsDir, file), 'utf8');
    }
  }
}
...
const WEATHER_ICONS = ${JSON.stringify(weatherIconFiles)};
const CARD_ICONS = ${JSON.stringify(cardIconFiles)};
const ALERT_ICONS = ${JSON.stringify(alertIconFiles)};
...
export default {
  async fetch(request, env, ctx) {
    ...
    // Serve vendored Meteocons fill weather icons (embedded at build time)
    if (url.pathname.startsWith('/icons/weather/')) {
      const iconName = url.pathname.split('/').pop();
      if (iconName && WEATHER_ICONS[iconName]) {
        return new Response(WEATHER_ICONS[iconName], {
          headers: {
            'Content-Type': 'image/svg+xml',
    ...
    // Serve vendored Meteocons static fill card header icons (embedded at build time)
    if (url.pathname.startsWith('/icons/cards/')) {
      const iconName = url.pathname.split('/').pop();
      if (iconName && CARD_ICONS[iconName]) {
        return new Response(CARD_ICONS[iconName], {
          headers: {
            'Content-Type': 'image/svg+xml',
            'Cache-Control': 'public, max-age=31536000, immutable',
          },
    ...
    // Serve vendored Meteocons fill alarm icons (embedded at build time)
    if (url.pathname.startsWith('/icons/alerts/')) {
      const iconName = url.pathname.split('/').pop();
      if (iconName && ALERT_ICONS[iconName]) {
        return new Response(ALERT_ICONS[iconName], {
          headers: {
            'Content-Type': 'image/svg+xml',
            'Cache-Control': 'public, max-age=31536000, immutable',
          },
