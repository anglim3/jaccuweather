...
If you regenerate the project, use `node ios/scripts/generate-xcodeproj.js` (it emits the flattened folder refs and the `JaccuweatherWidgets` extension). Do not point Copy Bundle Resources at the parent `Resources` directory.
...
**Free Apple ID / Personal Team.** App Groups cannot be provisioned, so the shared container is missing and the snapshot is never written. Touch and hold the widget, tap **Edit Widget**, and search for a city or enter latitude and longitude (for example `47.61, -122.33`). The extension loads that place from `api.open-meteo.com/v1/forecast`. The same configured place is used when a snapshot exists but is stale. A fresh App Group snapshot still wins when both are available.
...
| Widgets | Fresh App Group snapshot when the container exists (paid developer team). Otherwise the place chosen on the widget, from `api.open-meteo.com/v1/forecast` (current, a short hourly window, daily high/low). Search uses `geocoding-api.open-meteo.com` |
