...
The app and the `JaccuweatherWidgets` extension share the App Group `group.cloud.janglim.jaccuweather`. After a forecast refresh, the app writes the active place and current conditions there. The widget reads that file and reloads about every 20 minutes. It calls Open-Meteo only when the snapshot is older than about 40 minutes.

Install the app, open it once so a place loads, then:

1. Home Screen: long-press, tap **+**, search **Jaccuweather**, and add the small or medium widget.
2. Lock Screen: long-press, tap **Customize**, and add the circular or rectangular Jaccuweather accessory.
