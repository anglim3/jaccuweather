...
| Radar overlay | NWS WMS `nexrad-n0q-wmst` is still what the binary requests; that layer returns `LayerNotDefined`. Plan: [`docs/native-ios-radar.md`](../docs/native-ios-radar.md) |
| Global radar | Safari Ventusky link is still on the radar tab. The plan replaces it with in-app tiles and drops the link |
